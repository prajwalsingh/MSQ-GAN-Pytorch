...
def disc_hinge(dis_real, dis_fake):
	zeros     = torch.zeros_like(dis_real, dtype=torch.float32)
	real_loss = -1.0 * torch.mean( torch.min(zeros, -1.0 + dis_real) )
	fake_loss = -1.0 * torch.mean( torch.min(zeros, -1.0 - dis_fake) )
	return (real_loss + fake_loss)/2.0

def gen_hinge(dis_fake):
	fake_loss = -1.0 * torch.mean( dis_fake )
	return fake_loss
